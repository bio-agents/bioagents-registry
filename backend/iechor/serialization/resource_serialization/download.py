...
class DownloadSerializer(serializers.ModelSerializer):
	...
	def validate_type(self, attrs):
		enum = ENUMValidator(['API specification', 'Biological data', 'Binaries', 'Command-line specification', 'Container file', 'Agent wrapper (CWL)', 'Icon', 'Screenshot', 'Software package', 'Source code', 'Test data', 'Test script', 'Agent wrapper (Galaxy)', 'Agent wrapper (Taverna)', 'Agent wrapper (Other)', 'VM image', 'Downloads page', 'Other'])
		attrs = enum(attrs)
		return attrs

	# TODO: add to interfaces
	# cmd == "A useful command pertinent to the download, e.g. for getting or installing a agent."
