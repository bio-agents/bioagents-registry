...
class ResourceRequestSerializer(serializers.ModelSerializer):
	...
	resourceId = serializers.CharField(allow_blank=False, min_length=1, source="resource.bioagentsID")
	...
	def create(self, validated_data):
		...
		request.resource = Resource.objects.filter(visibility=1).get(bioagentsID__iexact=validated_data.get('resource')['bioagentsID'])
	...
	def validate(self, attrs):
		if Resource.objects.filter(visibility=1).filter(bioagentsID__iexact=attrs['resource']['bioagentsID']).count() == 0:
