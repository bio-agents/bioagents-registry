...
from iechor.models import Domain, DomainResource, DomainTag, DomainCollection, Resource
...
class DomainResourceSerializer(serializers.ModelSerializer):
	...
	bioagentsID = serializers.CharField(min_length=1, max_length=100, allow_blank=False, required=True, validators=[IsStringTypeValidator])	
	
	
	class Meta:
		model = DomainResource
		fields = ('name', 'bioagentsID')

	def validate_bioagentsID(self, b_id):
		return b_id.lower()
...
class DomainSerializer(serializers.ModelSerializer):
	...
	def create(self, validated_data):
		...
		seen = set()
		resources_list = [seen.add(obj['bioagentsID']) or obj for obj in resources_list if obj['bioagentsID'] not in seen]
		...
		for r in resources_list:
			agent_r = Resource.objects.filter(visibility=1, bioagentsID = r['bioagentsID'])
			if len(agent_r) == 1:
				r['name'] = agent_r[0].name
