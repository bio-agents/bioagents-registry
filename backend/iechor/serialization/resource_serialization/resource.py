...
from iechor.issues import EDAMTopicIssue, EDAMOperationIssue, EDAMDataIssue, EDAMFormatIssue, NoLicenseIssue, NoContactIssue, NoTOSIssue
...
class OtherIDSerializer(serializers.ModelSerializer):
	...
	def validate_value(self, attrs):
		...
		# The bioagentsCURIE must have at least one character, that's why the + is there
		# need access to multiple fields should merge the two validate_ functions into one validate()
		b = re.compile('^bioagents:[_\-.0-9a-zA-Z]+$', re.IGNORECASE | re.UNICODE)

		if not (d.search(attrs) or r.search(attrs) or c.search(attrs) or b.search(attrs)):
			raise serializers.ValidationError('The value has to be a DOI, RRID, cpe or bioagentsCURIE. Make sure you added the prefixes.')
		return attrs

	def validate_type(self, attrs):
		enum = ENUMValidator(['doi', 'rrid', 'cpe', 'bioagentsCURIE'])
		attrs = enum(attrs)
		return attrs

	def validate(self, data):
		if data.get("type") and data.get("value"):
			# check if super user , who is the only one allowed to change bioagents type of otherID
			if data["type"] == "bioagentsCURIE":
				try:
					is_superuser =  self.context['request'].user.is_superuser
					if is_superuser == False and self.context['request_type'] == 'POST':
				 		raise serializers.ValidationError('Only admin users can add \'bioagents:\'-type of otherID.')
				except (AttributeError, KeyError) as e: 
				# this means there is no context, so no PUT / POST request
					pass
				if not(data["value"].lower().startswith("bioagents:")):
			...
			if data["value"].lower().startswith("bioagents:"):
				try:
					is_superuser =  self.context['request'].user.is_superuser
					if is_superuser == False and self.context['request_type'] == 'POST':
				 		raise serializers.ValidationError('Only admin users can add \'bioagents:\'-type of otherID.')
...
# iechorPlatform
class iEchorPlatformSerializer(serializers.ModelSerializer):
	iechorPlatform = serializers.CharField(allow_blank=False, required=False)
	
	class Meta:
		model = iEchorPlatform
		fields = ('iechorPlatform',)

	def validate_iechorPlatform(self, attrs):
		enum = ENUMValidator(['Data', 'Agents', 'Compute', 'Interoperability', 'Training'])
		attrs = enum(attrs)
		return attrs

	def to_representation(self, obj):
		return obj.iechorPlatform
	...
	def to_internal_value(self, data):
		...
		return {"iechorPlatform":data}


# iechorNode
class iEchorNodeSerializer(serializers.ModelSerializer):
	iechorNode = serializers.CharField(allow_blank=False, required=False)
	
	class Meta:
		model = iEchorNode
		fields = ('iechorNode',)

	def validate_iechorNode(self, attrs):
	...
	def to_internal_value(self, data):
		...
		return {"iechorNode":data}

	def get_pk_field(self, model_field):
		return None

# iechorCommunity
class iEchorCommunitySerializer(serializers.ModelSerializer):
	iechorCommunity = serializers.CharField(allow_blank=False, required=False)
	
	class Meta:
		model = iEchorCommunity
		fields = ('iechorCommunity',)

	def validate_iechorCommunity(self, attrs):
...
class RelationSerializer(serializers.ModelSerializer):
	bioagentsID = serializers.CharField(allow_blank=False, required=True)
	type = serializers.CharField(allow_blank=True, required=True)
	
	class Meta:
		model = Relation
		fields = ('bioagentsID','type')

	def validate_bioagentsID(self, attrs):

		r = Resource.objects.filter(visibility=1,bioagentsID=attrs)

		if len(r) == 0:
			raise serializers.ValidationError('There is no resource with bioagentsID:' + attrs + ';The resource might have been deleted.')

		if len(r) > 1:
			raise serializers.ValidationError('DuplicateBioagentsIDError on resource with id:' + attrs + ';please contact our support team and paste the error in the message.')
...
class ResourceSerializer(serializers.ModelSerializer):
	...
	# instead of the id we use bioagentsID to identify the resources from the user point of view
	lookup_field = 'bioagentsID'

	# # need an alias so input has 'id' which is saved in the 'bioagentsID' field
	# id = serializers.CharField(read_only=True, source="bioagentsID");
	# id = serializers.CharField(allow_blank=False, max_length=50, min_length=1, source="bioagentsID", validators=[UniqueValidator(queryset=Resource.objects.filter(visibility=1), message="A resource with this ID already exists.")]);
	...
	# name = serializers.CharField(min_length=1, max_length=100, allow_blank=False, validators=[IsStringTypeValidator, UniqueValidator(queryset=Resource.objects.filter(visibility=1), message="The resource ID (bioagentsID) generated from this name already exists; Use a different name.")])
	...
	iechor_badge = serializers.IntegerField(read_only=True)
	...
	# iechorInfo = iEchorInfoSerializer(many=False, required=False)
	...
	agentType = AgentTypeSerializer(many=True, required=False, allow_empty=False)
	...
	iechorPlatform = iEchorPlatformSerializer(many=True, required=False, allow_empty=False)
	iechorNode = iEchorNodeSerializer(many=True, required=False, allow_empty=False)
	iechorCommunity = iEchorCommunitySerializer(many=True, required=False, allow_empty=False)
	...
	def validate_bioagentsID(self, attrs):
		...
		if not p.search(attrs):
			raise serializers.ValidationError('The bioagentsID can only contain letters, numbers or these characters: . - _ ~ ')
		if not p1.search(attrs):
			raise serializers.ValidationError('The bioagentsID can only start with letters and numbers')
		if attrs.endswith("."):
			raise serializers.ValidationError('The bioagentsID cannnot end with a dot')
		return attrs
	...
	def validate_confidence_flag(self, attrs):
		enum = ENUMValidator(['agent', 'high','medium', 'low', 'very low'])
		attrs = enum(attrs)
		return attrs
	...
	def create(self, validated_data):
		...
		iechorPlatform_list = validated_data.pop('iechorPlatform') if 'iechorPlatform' in list(validated_data.keys()) else []
		iechorNode_list = validated_data.pop('iechorNode') if 'iechorNode' in list(validated_data.keys()) else []
		iechorCommunity_list = validated_data.pop('iechorCommunity') if 'iechorCommunity' in list(validated_data.keys()) else []

		credit_dict = validated_data.pop('credit') if 'credit' in list(validated_data.keys()) else []
		# iechorInfo_dict = validated_data.pop('iechorInfo') if 'iechorInfo' in validated_data.keys() else []
		operatingSystem_list = uniq(validated_data, 'operatingSystem')
		agentType_list = uniq(validated_data, 'agentType')
		...
		# iechorInfo = None
		# if iechorInfo_dict:
		# 	iechorInfo = iEchorInfo.objects.create(**iechorInfo_dict)
		...
		validated_data['bioagentsCURIE'] = 'bioagents:' + validated_data['bioagentsID']

		# # if bioagentsID not passed in save(), then generate one out of name
		# if 'bioagentsID' not in validated_data:
		# 	# create id and randomize it if there are conflicts
		# 	bioagentsID = re.sub(r"[^a-zA-Z0-9_. -]*", "", validated_data['name'])
		# 	bioagentsID = re.sub(r"[ ]+", "_", bioagentsID)
		# 	if len(Resource.objects.filter(visibility=1, bioagentsID__iexact=bioagentsID)) > 0 or bioagentsID in settings.RESERVED_URL_KEYWORDS:
		# 		raise CustomError('The name you provided clashes with an existing bioagentsID. Please provide a different name.', 'bioagentsID', status_code=status.HTTP_400_BAD_REQUEST)
		# 		# bioagentsID = initialbioagentsID + str(randint(1000, 9999))
		# 		# count += 1
		# 		# if count > 9998:
		# 		# 	raise CustomError('ID could not be generated', 'id', status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
		# 	validated_data['bioagentsID'] = bioagentsID
		# 	validated_data['bioagentsCURIE'] = 'bioagents:' + bioagentsID
		...
		for iechorPlatform in iechorPlatform_list:
			iEchorPlatform.objects.create(resource=resource, **iechorPlatform)

		for iechorNode in iechorNode_list:
			iEchorNode.objects.create(resource=resource, **iechorNode)

		for iechorCommunity in iechorCommunity_list:
			iEchorCommunity.objects.create(resource=resource, **iechorCommunity)
		...
		for agentType in agentType_list:
			AgentType.objects.create(resource=resource, **agentType)
...
class ResourceUpdateSerializer(ResourceSerializer):
	...
	#id = serializers.CharField(source="bioagentsID", read_only=True)
	#curie = serializers.CharField(source="bioagentsCURIE", read_only=True)

	bioagentsID = serializers.CharField(min_length=1, max_length=100, allow_blank=False, validators=[IsStringTypeValidator,])
	...
	# initially the name generates the bioagentsID
	# now technically they can change the agent name
	...
	class Meta:
		model = Resource
		#exclude = ('id', 'bioagentsID', 'owner','name', 'curie')
...
# basic information for agent list
class AgentListResourceSerializer(ResourceSerializer):
	#bioagentsID = serializers.CharField(source="bioagentsID", read_only=True)

	class Meta:
		model = Resource
		fields = ('bioagentsID', 'name', 'version', 'additionDate', 'lastUpdate', )
