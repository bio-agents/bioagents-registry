...
class XMLSchemaParser(BaseParser):
	...
	def parse(self, stream, media_type=None, parser_context=None):
		...
		try:
			...
			# redundant call if single-agent.xsd is used, still useful just in case
			if len(xml_tree) > 1:
			...
			raise ParseError('XML validation error - %s -- Note that you can only register one agent at a time.' % e)
