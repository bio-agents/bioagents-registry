...
def process_domain(domain_object):
	"""Obtain a union of agents from existing domain resources / agents and also regular agents."""

	domain_data = DomainSerializer(domain_object).data

	# get OrderedDict (name and bioagentsID) of resources that are in the domain
	domain_resources_dict = OrderedDict()
	for r in domain_data['resources']:
		domain_resources_dict[r['bioagentsID']] = r
 
	# get OrderedDict (name and bioagentsID) of resources that are annotated with a list of collections (coming from collections tagged in domains)
	resource_list = []
	for c in domain_data['collection']:
		...
		resource_list.extend([
			OrderedDict([('name', c.resource.name), ('bioagentsID', c.resource.bioagentsID)]) 
			for c in collection_objects if c.resource.visibility == 1
		])
	
	# union the resources objects (also hash the same bioagentsID sometimes which is good because it updates agent names in domains as a side-effect )
...
class Command(BaseCommand):
	"""Synchronize domains based on agent annotations (e.g. collections)"""

	help = 'Synchronize domains based on agent annotations (e.g. collections)'
