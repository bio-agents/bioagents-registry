...
from iechor.view.resource import es
...
class UsedTermsList(APIView):
	...
	def get(self, request, ontology, format=None):
		...
		if ontology == 'name':
		...
		elif ontology == 'agentType':
			fields = ['agentType.raw']
			...
			fields = ['topic.term.raw', 'function.operation.term.raw', 'name.raw', 'function.input.data.term.keyword', 'function.input.format.term.keyword', 'function.output.data.term.keyword', 'function.output.format.term.keyword', 'agentType.raw', 'language.raw', 'accessibility.keyword', 'cost.raw', 'license.raw', 'credit.name.keyword', 'collectionID.raw','name.raw']
		else:
