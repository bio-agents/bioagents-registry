...
class IssueView(APIView):
	...
	def post(self, request, bioagentsID, issueId=None):
		if not (bioagentsID):
			return Response({'data': None, 'text': 'Missing id.'}, status=status.HTTP_400_BAD_REQUEST)
		...
		i = Issue(
			issue_type=i_type,
			issue_state=i_state,
			resource_bioagentsID=bioagentsID,
			comment=comment,
