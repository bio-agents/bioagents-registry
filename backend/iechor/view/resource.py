...
from iechor.renderers import XMLSchemaRenderer, JSONLDRenderer
...
from iechor.parsers import XMLSchemaParser
from iechor.tasks import notify_admins
from iechor.tasks import notify_resource_request
from iechor.tasks import create_ecosystem_agent, update_ecosystem_agent, delete_ecosystem_agent
import json
...
def check_update_confidence_flag(resource_confidence_flag, request_confidence_flag):
	# cannot change confidence_flag to high, medium, low, very low once it's been set to None or 'agent'
	if (resource_confidence_flag == 'agent' or resource_confidence_flag == None) and (request_confidence_flag != None and request_confidence_flag != 'agent'):
		return False			
	return True
...
class ResourceList(APIView):
	...
	def get(self, request, format=None):
		...
		searchLogger = iechor_logging.SearchLogger(query_dict)
		searchLogger.commit()
		...
		if domain:
			...
			if domain_count > 0:
				...
				domain_resources = set([(x['bioagentsID']) for x in domain_result['resources']])
				
				# lowercase the bioagentsIDs so that it matches the elastic analyzer
				domain_ids = [rid.lower() for rid in list(domain_resources)]
				query_struct['query']['bool']['must'].append({"terms":{"bioagentsID": domain_ids}})
		...
		# 		domain_resources = set(map(lambda x: (x['bioagentsID']), domain_result['resources']))

		# 		# get touples of returned agents
		# 		returned_resource = set(map(lambda x: (x['bioagentsID']), results))
				
		# 		# if the query is just of the subdomain (e.g. domain=covid-19)
		# 		# 	and no other search terms are provided
		# 		#	then the total counts value "count" is the number of the agents in the domain
		...
		# 		if len(diff) > 0:
		# 			results = [t for t in results if t['bioagentsID'] in diff]
		# 		else:
	...
	def post(self, request, format=None):
		...
		if serializer.is_valid():
			if not(request.user.is_superuser) and request.data.get('confidence_flag') and request.data.get('confidence_flag') != 'agent':
				return Response({"detail": 'Error: Only superusers can add entries with a confidence score other than \'agent\'.'}, status=status.HTTP_403_FORBIDDEN)
			...
			# issue_function(Resource.objects.get(bioagentsID=serializer.data['bioagentsID'], visibility=1), request.user)
			es.index(index=settings.ELASTIC_SEARCH_INDEX, doc_type='_doc', body=serializer.data)
			# notify_admins(serializer.data['bioagentsID'], 'Agent CREATE', 'A agent was created.')
			
			if settings.GITHUB_ECOSYSTEM_ON:
				create_ecosystem_agent(serializer.data, str(request.user))
...
class DisownResourceView(APIView):
	...
	def get_object(self, bioagentsID):
		try:
			obj = Resource.objects.filter(visibility=1).get(bioagentsID__iexact=bioagentsID)
	...
	def post(self, request, bioagentsID, format=None):
		resource = self.get_object(bioagentsID)
...
class ResourceDetail(APIView):
	...
	def get(self, request, bioagentsID, format=None):
		resource = self.get_object(bioagentsID)
	...
	def process_request_for_otherID(self, request, rID):
		...
		if not(request.user.is_superuser) and request.data.get('otherID') and isinstance(request.data['otherID'],list):
			for oID in list(request.data['otherID']):
				...
				if oID.get("value") and oID["value"].lower().startswith("bioagents:"): 
					#remove if value is for "bioagents" otherID
					request.data['otherID'].remove(oID)


		# retrieve otherID of type bioagents, if any, from current version of agent
		# and add them to the request object
		o_objects = OtherID.objects.filter(resource_id=rID,value__startswith='bioagents:')
		if len(o_objects) > 0:
	...
	def put(self, request, bioagentsID, format=None):
		resource = self.get_object(bioagentsID)
		...
		if serializer.is_valid():
			# only superusers can change confidence_flag from agent to something else
			if not(request.user.is_superuser) and not(check_update_confidence_flag(resource.confidence_flag, request.data.get('confidence_flag'))):
				return Response({"detail": 'Error: Cannot change confidence_flag once it has been set to describe a agent.'}, status=status.HTTP_403_FORBIDDEN)
			...
			# attempt on the bioagentsID or bioagentsCURIE
			# Note that we need throw errors here we need to throw them before
			# makeing the old resource visibility 0 (see above)
			serializer.save(
				bioagentsID=resource.bioagentsID, 
				bioagentsCURIE=resource.bioagentsCURIE, 
				additionDate=resource.additionDate, 
				owner=resource.owner, 
				was_id_validated=is_id_valid
			)
			# issue_function(Resource.objects.get(bioagentsID=serializer.data['bioagentsID'], visibility=1), str(resource.owner))
				...
				"query": {
					"bool" : {
						"must": [
							{
								"match": {
									"bioagentsID": resource.bioagentsID.lower()
								}
							}
						]
					}
			...
			# notify_admins(serializer.data['bioagentsID'], 'Agent UPDATE', 'A agent was updated.')
			if settings.GITHUB_ECOSYSTEM_ON:
				update_ecosystem_agent(serializer.data, str(request.user))
			return Response(serializer.data)
		
		return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

	def delete(self, request, bioagentsID, format=None):
		if request.user.is_superuser:
			...
			notify_admins(bioagentsID, 'Agent DELETE', 'A agent was was deleted.')
			if settings.GITHUB_ECOSYSTEM_ON:
				delete_ecosystem_agent(bioagentsID, str(request.user))
...
# 	def get_list(self, bioagentsID):
# 		try:
# 			obj = Resource.objects.filter(visibility=1, bioagentsID__iexact=bioagentsID)
# 			self.check_object_permissions(self.request, obj)
# 			return obj
# 		except Resource.DoesNotExist:
# 			raise Http404

# 	def get(self, request, bioagentsID, version=None, format=None):
# 		resource = self.get_list(bioagentsID)
...
class ResourceUpdateValidator(APIView):
	...
	def get_object(self, bioagentsID):
		try:
			obj = Resource.objects.filter(visibility=1).get(bioagentsID=bioagentsID)
			self.check_object_permissions(self.request, obj)
			return obj
		except Resource.DoesNotExist:
			raise Http404
