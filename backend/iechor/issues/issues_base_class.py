import datetime
from iechor.models import IssueType, IssueState, Issue, Resource
...
class IssueBaseClass(object):
	...
	def report(self):
		for agent in self.qset:
			# get the detected issues for the agent
			detected_issues = self.detect(agent)
			# get the existing issues of this type
			existing_issues = Issue.objects.filter(
				issue_type=self.issue_type,
				resource_bioagentsID=agent.bioagentsID,
				resource_versionId=agent.versionId
			)
