...
# 	# purge
# 	try:
# 		resp = es.indices.delete(index='iechor')
...
# 		es.index(index='iechor', doc_type='agent', body=resource)
...
# 		link=urlresolvers.reverse("admin:iechor_resource_change", args=[obj.resource.id])
...
# 		link=urlresolvers.reverse("admin:iechor_function_change", args=[obj.function.id])
...
# 		link=urlresolvers.reverse("admin:iechor_resource_change", args=[obj.function.resource.id])
# 		return u'<a href="%s">%s</a>' % (link,resource[0])
# 	link_to_resource.allow_tags=True
...
# 		link=urlresolvers.reverse("admin:iechor_resource_change", args=[resource[0].id])
# 		return u'<a href="%s">%s</a>' % (link,resource[0])
# 	link_to_resource.allow_tags=True
...
# 		link=urlresolvers.reverse("admin:iechor_resource_change", args=[resource.id])
...
# class iEchorInfoAdmin(SuperModelAdmin):
# 	search_fields = ["status", "node", "resource__name"]
# 	list_display = ["status", "node", "link_to_resource"]
# 	# inlines = []

# 	def link_to_resource(self, obj):
# 		resource = Resource.objects.get(iechorInfo=obj.id)
# 		link=urlresolvers.reverse("admin:iechor_resource_change", args=[resource.id])
# 		return u'<a href="%s">%s</a>' % (link,resource)
# 	link_to_resource.allow_tags=True
...
# 	# get only visible resources
# 	def get_queryset(self, request):
# 		qs = super(iEchorInfoAdmin, self).get_queryset(request)
# 		return qs.filter(resource__visibility=1)

# 	def get_urls(self):
# 		urls = super(iEchorInfoAdmin, self).get_urls()
# 		my_urls = patterns("",
# 			url(r"^es_regenerate/$", es_regenerate)
# 		)
# 		return my_urls + urls

# admin.site.register(iEchorInfo, iEchorInfoAdmin)

# class iEchorInfoInline(SuperInlineModelAdmin, StackedInline):
# 	model = iEchorInfo
# 	# inlines = []
# 	can_delete = True
# 	extra = 0
...
# class AgentTypeAdmin(SuperModelAdmin):
# 	search_fields = ["name", "resource__name"]
# 	list_display = ["name", "link_to_resource"]
# 	# inlines = []
...
# 	# get only visible resources
# 	def get_queryset(self, request):
# 		qs = super(AgentTypeAdmin, self).get_queryset(request)
# 		return qs.filter(resource__visibility=1)

# 	def get_urls(self):
# 		urls = super(AgentTypeAdmin, self).get_urls()
# 		my_urls = patterns("",
# 			url(r"^es_regenerate/$", es_regenerate)
# 		)
# 		return my_urls + urls
...
# class AgentTypeInline(SuperInlineModelAdmin, StackedInline):
# 	model = AgentType
# 	# inlines = []
# 	can_delete = True
# 	extra = 0
...
# 	inlines = [TopicInline, FunctionInline, OperatingSystemInline, AgentTypeInline, LanguageInline, CollectionIDInline, UsesInline, ContactInline]
# 	search_fields = ['name']
