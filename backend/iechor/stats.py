from iechor.models import Resource, StatsData
...
class StatsInfo():
	...
	def generateTotalAnnotationsCount(self, stat):
		return {"date": stat.date, 
				...
				"agentTypeAnnotationCount": stat.agentTypeAnnotationCount,
	...
	def totalAnnotationCount(self, upperDateLimit=datetime.today()):
		...
		annotationInfo.update(self.agentTypeAnnotationCount(upperDateLimit))
	...
	def agentTypeAnnotationCount(self, upperDateLimit=datetime.today()):
		return {'agentTypeAnnotationCount':  AgentType.objects.filter(resource__visibility=1, additionDate__lt=upperDateLimit).count()}

	def uniqueIDAnnotationCount(self, upperDateLimit=datetime.today()):
		return {'uniqueIDAnnotationCount': Resource.objects.filter(visibility=1, additionDate__lt=upperDateLimit).exclude(bioagentsID="").count()}
	...
	def topSearchStats(self, limit):
		return iechor_logging.search_log_json_data()[:limit]
