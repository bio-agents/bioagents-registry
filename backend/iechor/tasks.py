...
from iechor.models import StatsData
import iechor.stats as stats
from iechor.models import *
from iechor.issues import *
...
import json
import requests
from iechor.publication_metadata import update_publication
...
def backup_stats_with_date(upperDateLimit=datetime.today()):
    ...
    statsData.agentTypeAnnotationCount = statsData.data["totalAnnotationCount"]["agentTypeAnnotationCount"]
...
# import the ecosytem_handle_agent_operations which handles the create, update and delete ecosystem operations
from iechor.bioagents_to_github import ecosytem_handle_agent_operations

@background(schedule=30)
def create_ecosystem_agent(agent, user):
    # We might need to clean the agent before if we don't accept "empty" properties
    ...
    ecosytem_handle_agent_operations(user, agent_json, 'CREATE')
    

@background(schedule=30)
def update_ecosystem_agent(agent, user):
    # We might need to clean the agent before if we don't accept "empty" properties
    ...
    ecosytem_handle_agent_operations(user, agent_json, 'UPDATE')

@background(schedule=30)
def delete_ecosystem_agent(agent_id, user):
    # Convert to string and pretty format a minimal agent JSON with only the bioagentsID
    agent_json = json.dumps(
        {"bioagentsID":agent_id},
        sort_keys=True,
        indent=4,
        separators=(',', ': ')
    )

    ecosytem_handle_agent_operations(user, agent_json, 'DELETE')