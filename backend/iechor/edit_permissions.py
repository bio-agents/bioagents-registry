from iechor.models import EditPermission, EditPermissionAuthor, Resource
from iechor.permissions import IsOwnerOrReadOnly
...
class EditPermissions(APIView):
	...
	def getResource(self, id):
		obj = Resource.objects.filter(visibility=1).get(bioagentsID__iexact=id)
	...
	def get(self, request):
		...
		resource = self.getResource(parameters["id"])
		editpermission = resource.editPermission
		return Response({'id': resource.bioagentsId, 'type': editpermission.type})
