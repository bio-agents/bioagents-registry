...
from .bioagents_handler import BioAgentsData
...
class GithubAgentHandler:
    ...
    # For now we use one main GitHub account, e.g. bioagents-bot for working
    #   with all the files
    ...
    def __get_filename(self,agent_id):
        return ".".join([agent_id, self.BIOTOOLS_EXTENSION])

    def __get_branch_name(self, username, agent_id, delete = False):
    ...
    def __get_filepath(self, agent_id):
        return os.path.join(self.ROOT_GITHUB_FOLDER, agent_id, self.__get_filename(agent_id))
    
    # Maybe in commit message at a timestamp from the agent
    def __create_commit_message(self, method, username, agent_id):
        return '{m} agent with id: {id} by user: {u} at: {at}'.format(
            m = method, 
            id = agent_id, 
            u = username,
    ...
    def __get_sha(self, file_path, branch_name):
        ...
        try:
            ...
            raise EcosystemException(
                'Could not update agent, either filepath or branch do not exist.' + str(e),
                branch = branch_name,
                filepath = file_path,
                sha = None,
                username = self.__bt.username,
                agent_id = self.__bt.agent_id
            )
    ...
    def __create_data(self, branch_name, new_file_path, custom_message = None, custom_content = None): 
        ...
        Raises AgentCreationException if file cannot be created
        """
        ...
        try:
            c = self.__repo.create_file(
                ...
                content = custom_content if custom_content else self.__bt.agent_json,
                branch=branch_name
            )
        except GithubException as e:
    ...
    def __update_data(self, update_branch_name, update_file_path, file_sha):        
        ...
        Raises AgentUpdateException on update fail.
        """
        commit_message = self.__create_commit_message(
            self.UPDATE, 
            self.__bt.username, 
        ...
        try:
            u = self.__repo.update_file(
                ...
                message = commit_message,
                content = self.__bt.agent_json,
                branch=update_branch_name,
                sha=file_sha
            )
        except GithubException as e:
            raise AgentUpdateException(
    ...
    def __create_pull_request(self, pr_type, username, agent_id, branch_name):
        """
        Creates a pull request for one of the: CREATE, UPDATE, DELETE agent operations.
        Raises one of: 
        AgentCreationException, AgentUpdateException, AgentDeleteException, EcosystemException on fail
        """
        pr_title = '{type} agent with id: {id}'.format(type=pr_type, id=agent_id)
        pr_body = """
        {type} agent with id: {id}
        by user: {user}
        at: {at}
        """.format(
            type=pr_type, 
        ...
        try: 
            ...
            if pr_type == self.CREATE:
                raise AgentCreationException(
                        'Pull request creation failed for agent CREATE operation.'.format(type=pr_type) + str(e),
                        branch = branch_name,
                        filepath = self.__filepath,
                        sha = None,
                        username = username,
                ...
                raise AgentUpdateException(
                        'Pull request creation failed for agent UPDATE operation.'.format(type=pr_type) + str(e),
                        branch = branch_name,
                        filepath = self.__filepath,
                        sha = None,
                        username = username,
                ...
                raise AgentDeleteException(
                        'Pull request creation failed for agent DELETE operation.'.format(type=pr_type) + str(e),
                        branch = branch_name,
                        filepath = self.__filepath,
                        sha = None,
                        username = username,
    ...
    def __load_data(self, bt, delete = False):
        self.__bt = bt
        self.__branch = self.__get_branch_name(bt.username, bt.agent_id, delete)
        self.__filepath = self.__get_filepath(bt.agent_id)
        self.__time = datetime.utcnow().strftime('UTC time: %Y-%m-%d %H:%M:%S')

    def create_agent(self, bioagents_data):
        if not(self.__logged_in):
            raise EcosystemLoginException(
                self.__username, 
                self.__repo_name, 
                'Bad Github user credetials at agent create time.'
            )

        # Load some class level variables for convenience in Exception handling
        self.__load_data(bioagents_data)

        new_branch_name = self.__branch
        new_file_path  = self.__filepath

        # Exception brach already exists on agent creation time
        if self.__branch_exists(new_branch_name):
            raise AgentCreationException(
                'Branch already exists at agent creation time.',
        ...
        # Exception that there is already a PR with this branch name on agent creation time
        if self.__pull_request_exists(new_branch_name):
            raise AgentCreationException(
                'There is already a PR with for the branch name at agent creation time.',
                branch = new_branch_name,
                filepath = new_file_path,
                sha = None,
                username = self.__bt.username,
        ...
        if self.__file_exists(new_file_path, new_branch_name):
            raise AgentCreationException(
                'The file already exists on branch: {b} at agent creation time.'.format(b=new_branch_name),
                branch = new_branch_name,
                filepath = new_file_path,
                sha = None,
                username = self.__bt.username,
        ...
        if self.__file_exists(new_file_path, self.MASTER_BRANCH):
            raise AgentCreationException(
                'The file already exists on master branch: {b} at agent creation time.'.format(b=self.MASTER_BRANCH),
                branch = new_branch_name,
                filepath = new_file_path,
                sha = None,
                username = self.__bt.username,
    ...
    def update_agent(self, bioagents_data):
        if not(self.__logged_in):
            raise EcosystemLoginException(
                self.__username, 
                self.__repo_name, 
                'Bad Github user credetials at agent update time.'
            )
        ...
        #   we catch it here and raise AgentUpdateException
    ...
    #   so that we draw more attention to the fact that a agent is being deleted
    #   In this way we also separate the deletion operations from create and update
    def delete_agent(self, bioagents_data):
        """
        Delete the agent from the GitHub ecosytem
        """
        ...
        if not(self.__logged_in):
            raise EcosystemLoginException(
                self.__username, 
                self.__repo_name, 
                'Bad Github user credetials at agent delete time.'
            )

        # Load some class level variables for convenience in Exception handling
        self.__load_data(bioagents_data, delete = True)

        # this is the branch for the whole operations
        delete_branch_name = self.__branch
        delete_file_path = self.__filepath
        
        # Exception brach already exists on agent delete time
        if self.__branch_exists(delete_branch_name):
            raise AgentDeleteException(
                'Branch already exists at agent delete time.',
                branch = delete_branch_name,
                filepath = delete_file_path,
                sha = None,
                username = self.__bt.username,
        ...
        if self.__pull_request_exists(delete_branch_name):
            raise AgentDeleteException(
                'There is already a PR with for the branch name at agent delete time.',
                branch = delete_branch_name,
                filepath = delete_file_path,
                sha = None,
                username = self.__bt.username,
        ...
        # Rare case of when user creates the agent and immediately deletes it
        # (This won't happen unless we allow users to delete the agent)
        ...
        if not self.__file_exists(delete_file_path, self.MASTER_BRANCH):
            ...
            if not self.__branch_exists(regular_non_delete_branch) or \
                not self.__pull_request_exists(regular_non_delete_branch):
                   raise AgentDeleteException(
                        'The file does not exist on master branch: {b} or on any other user-related branch at agent delete time.'.format(b=self.MASTER_BRANCH),
            ...
            # the file is not on master and also not on a regular CREATE/UPDATE (non delete) branch
                ...
                raise AgentDeleteException(
                    'The file does not exist on any valid branches: {b} at agent delete time.'.format(b=self.MASTER_BRANCH),
        ...
        if not self.__file_exists(delete_file_path, delete_branch_name):
            raise AgentDeleteException(
                'The file does not exist on branch: {b} at agent delete time.'.format(b=delete_branch_name),
                branch = delete_branch_name,
                filepath = delete_file_path,
                sha = None,
                username = self.__bt.username,
        ...
        if self.__file_exists(delete_file_path, delete_branch_name):
            raise AgentDeleteException(
                'The file was not deleted on branch: {b} at agent delete time.'.format(b=delete_branch_name),
                branch = delete_branch_name,
                filepath = delete_file_path,
                sha = file_sha,
                username = self.__bt.username,
    ...
    #   that are not used for handling agents and those we can't delete
    # We can have either a naming convention for branches that are 
    #   associated with agents and those we delete if the branch has the 'right' name
