...
#   Because we don't technically need the bio.agents JSON data when we delete the agent
...
class BioAgentsData(object):
    ...
    def __init__(self, username, agent_json_string, delete = False, agent_id = None):
        ...
        # We can look in bio.agents for this username if we really want to make sure
        #   but since this code is for now called from bio.agents backend we don't really have to
        if not(username):
            raise BioAgentsException(username, agent_json_string, 'No valid bio.agents username was provided.')
    ...
    def validate_agent(self):
        ...
        #   from bio.agents after validation has been done
