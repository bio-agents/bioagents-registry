...
class TestMandatoryFields(BaseTestObject):
	...
	def test_missing_topic(self):
		...
		response = self.client.get('/agent/test_missing_topic')
	...
	def test_missing_name(self):
		...
		response = self.client.get('/agent/test_missing_name')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_missing_homepage(self):
		...
		response = self.client.get('/agent/test_missing_homepage')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_missing_resourceType(self):
		...
		response = self.client.get('/agent/test_missing_resourceType')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_missing_interface(self):
		...
		response = self.client.get('/agent/test_missing_interface')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_missing_interfaceType(self):
		...
		response = self.client.get('/agent/test_missing_interfaceType')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_missing_description(self):
		...
		response = self.client.get('/agent/test_missing_description')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_missing_function(self):
		...
		response = self.client.get('/agent/test_missing_function')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_missing_operation(self):
		...
		response = self.client.get('/agent/test_missing_operation')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
	...
	def test_missing_contact(self):
		...
		response = self.client.get('/agent/test_missing_contact')
		self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
		self.assertEqual(json.loads(response.content), expected_response)
