...
.controller('AgentEditController', ['$scope', '$controller', '$state', '$stateParams', 'Ontology', 'Attribute', 'CheckUserEditingRights', 'User', '$timeout', 'UsedTerms','$q','filterFilter', function($scope, $controller, $state, $stateParams, Ontology, Attribute, CheckUserEditingRights, User, $timeout, UsedTerms, $q, filterFilter ) {
	...
	$scope.downloadTypeOptions = [
		...
		{value: "Agent wrapper (CWL)", text: "Agent wrapper (CWL)"},
...
.controller('AgentUpdateController', ['$scope', '$controller','$timeout','$state', '$stateParams', 'Agent', 'AgentUpdateValidator', 'Covid', 'CommunityCollection', function($scope, $controller, $timeout, $state, $stateParams, Agent, AgentUpdateValidator, Covid, CommunityCollection) {
	...
	$scope.naviagateToAgent = function(bioagentsID) {
		$timeout(function() {
			if (confirm("Make sure you save before navigating away! Are you sure you want to leave? ")) {
				$state.go('agent', {id: bioagentsID}, {reload: true});
			}
		},100);
	}
...
.controller('AgentCreateController', ['$scope', '$controller', '$timeout', 'AgentListConnection', 'AgentCreateValidator', 'User', '$stateParams', 'Covid', 'CommunityCollection',function($scope, $controller, $timeout, AgentListConnection, AgentCreateValidator, User, $stateParams, Covid, CommunityCollection){
	// inherit common controller
	$controller('AgentEditController', {$scope: $scope});
	$scope.orderby = 'text';
	// sets which controller is in use, so the HTML can adapt
	...
	$scope.bioagentsIDDisabled = true;
	...
	$scope.makeIdURLSafe = function(value) {
		if (typeof value != 'undefined' && $scope.bioagentsIDDisabled) {
			$scope.software.bioagentsID = value.replace(/[^a-zA-Z0-9_~ .-]*/g,'').replace(/[ ]+/g, '_').toLowerCase();
		}else if ($scope.bioagentsIDDisabled){
			$scope.software.bioagentsID = "";
		}
		
	}

	$scope.editIdToggleButtonClick = function () {
		$scope.bioagentsIDDisabled = !$scope.bioagentsIDDisabled;
		$scope.makeIdURLSafe($scope.software.name);

		if ($scope.bioagentsIDDisabled){
			$scope.editIdButtonText = 'Edit ID';
		}else{
	...
	$scope.validateButtonClick = function() {
		$timeout(function() {
			$scope.sendResource(AgentCreateValidator.save, $scope.validationProgress, false, 'create-validate');
		},100);
	}

	$scope.registerButtonClick = function() {
		if (confirm("Are you sure you want to save the resource?\nOnce saved the agent ID cannot be changed!")) {
			$timeout(function() {
