...
// Agent function to get dtnode from the event target:
...
//          noop(); // keep JSLint happy
		...
		if(!nodeTitle){
			var agenttip = data.agenttip ? ' title="' + data.agenttip.replace(/\"/g, '&quot;') + '"' : '',
				href = data.href || "#";
			if( opts.noLink || data.noLink ) {
				nodeTitle = '<span style="display:inline-block;" class="' + opts.classNames.title + '"' + agenttip + '>' + data.title + '</span>';
//              this.tree.logDebug("nodeTitle: " + nodeTitle);
			} else {
				nodeTitle = '<a href="' + href + '" class="' + opts.classNames.title + '"' + agenttip + '>' + data.title + '</a>';
			}
		}
		res += nodeTitle;
		return res;
...
//          return a.data.title === b.data.title ? 0 : a.data.title > b.data.title ? 1 : -1;
	...
	setLazyNodeStatus: function(lts, opts) {
		var agenttip = (opts && opts.agenttip) ? opts.agenttip : null,
...
//      this.tree.logDebug("dtnode.toggleSelect() - %o", this);
	...
	_loadContent: function() {
		try {
			...
			this.setLazyNodeStatus(DTNodeStatus_Error, {agenttip: ""+e});
		}
	},
...
//              self.tree.logDebug("appendAjax().success");
			...
			error: function(jqXHR, textStatus, errorThrown){
				...
				self.setLazyNodeStatus(DTNodeStatus_Error, {info: textStatus, agenttip: "" + errorThrown});
				}
		});
		$.ajax(options);
	},
...
//              self.logDebug("%o", title);
			...
			if( $li.attr("title") ){
				data.agenttip = $li.attr("title"); // overrides <a title='...'>
			}
			if( $li.attr("id") ){
				data.key = "" + $li.attr("id");
			}
...
/*******************************************************************************
 * Agents in ui.dynatree namespace
 */
$.extend($.ui.dynatree, {
	/** @type {String} */
	version: "DEVELOPMENT",
...
$.ui.dynatree.nodedatadefaults = {
	...
	agenttip: null, // Show this popup text.
