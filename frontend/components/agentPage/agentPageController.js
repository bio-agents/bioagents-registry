...
.controller('AgentPageController', ['$scope', '$state', '$timeout', '$stateParams', 'Agent', 'User', 'CheckUserEditingRights', 'ResourceRequestProvider', 'ngMeta', 'Query', 'Covid', '$http',  function($scope, $state, $timeout, $stateParams, Agent, User, CheckUserEditingRights, ResourceRequestProvider, ngMeta, Query, Covid, $http) {
	...
	$scope.requestOwnership = function(resourceId) {
		if (confirm("We recommend that only the agent developers/maintainers request ownership.\nAll other users should request edit-rights.\nAre you sure you want to request ownership of this agent?")){
	...
	// Call the API t retrieve and inject JSON-LD semantic annotations (Bioschemas agent profile)
	$scope.software.$promise.then(function(data) {
     		// console.log('Success: '+JSON.stringify(data));
     		// console.log("calling API "+"/api/"+data.bioagentsID+"?format=jsonld");
			$http.get("/api/"+data.bioagentsID+"?format=jsonld").then(function(response) {
...
.directive("agentPageNewLinkCallout", function(){
	return {
		restrict: 'A',
		scope:{
			'url': '=?',
		...
		templateUrl: 'components/agentPage/partials/agentPageNewLinkCallout.html',
		replace: true
	};
})
